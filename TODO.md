...
- [ ] Prompt mẫu cho Trang chủ (điều chỉnh theo style guide của nhóm):
  ```
  Design a homepage for an online rental room web app named "ReadMore" (Vietnamese).
...
- [ ] **Effect 2** — cập nhật tab title (dependency `[totalItems]`):
  ...
  useEffect(() => {
    document.title = totalItems > 0 ? `(${totalItems}) ReadMore` : 'ReadMore Rental Room';
    return () => { document.title = 'ReadMore Rental Room'; }; // cleanup khi unmount
  }, [totalItems]);
  ```
...
**Setup store [A]**
- [x] Cài: `npm install @reduxjs/toolkit react-redux`
- [x] Tạo `src/store/cartSlice.js` với `createSlice`:
  ```js
  ...
  ```
- [x] Tạo `src/store/index.js`: `configureStore({ reducer: { cart: cartReducer } })`
- [x] Bọc App trong `<Provider store={store}>` tại `main.jsx` (thay cho `<CartProvider>`)
...
- [x] Header: `const totalItems = useSelector(selectTotalItems)` — xóa `useCart()`
- [x] BookCard: `const dispatch = useDispatch(); dispatch(addToCart(book))`
- [x] CartPage: `useSelector(state => state.cart.items)` + dispatch `updateQuantity`/`removeFromCart`
...
chore: week10 - build, deploy Vercel v1.0, finalize ReadMore Rental Room (main)
```
